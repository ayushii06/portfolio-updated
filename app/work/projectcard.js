  ...
  return (
    <>
      <div
        ...
        className="size-full bg-repeat bg-[length:35px_35px]"
      >
        <div className="size-full rounded-3xl bg-gradient-to-tr from-zinc-950 via-zinc-950/70 to-zinc-950">
          <div className=" my-12 border border-neutral-800 rounded-3xl p-4 sm:p-6 shadow-xl relative hover:shadow-2xl transition-all duration-300 ">
            {/* Project Image */}
            <GlowingEffect
              spread={60}
              proximity={100}
            ...
            <div className="relative w-full h-36 sm:h-56 rounded-2xl overflow-hidden mb-5 border border-neutral-800">
            ...
            {stacks?.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {stacks.map((stack, index) => (
                  <span
                    key={index}
                    className="text-[10px] sm:text-xs bg-neutral-800 text-neutral-200 px-3 py-1 rounded-full border border-neutral-700"
                  >
                    {stack}
                  </span>
                ))}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {preview && (
                <Link
                  href={preview}
                  target="_blank"
                  className="flex-1 flex items-center justify-center gap-2 bg-white text-black py-2 rounded-xl text-xs sm:text-sm font-medium hover:opacity-90 transition"
                >
                  <Link2Icon size={16} />
                  Website
                </Link>
