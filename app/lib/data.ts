import clgpng from '../public/college.png';
import entrance from '../public/exam.png';
import school from '../public/school.png';

const experiencesData = [
  {
    date: '2022-26',
    title: 'Rajiv Gandhi Institute of Petroleum Technology , India',
    location: 'B.Tech in Computer Science and Design Engineering',
    description: 'Currenlty pre-final year undergrate student at RGIPT, India. SPI : 8.89/10.0 (Till 5th Sem) . Selected as MLSA at Microsoft in first year. ',
    icon : "https://res.cloudinary.com/drg0umzcb/image/upload/v1729763471/shicoImages/mo9jbdobtalnw9jww70p.png"

  },
  {
    date: 'August,2022',
    title: 'Joint Entrance Examination (JEE) Advanced',
    location: 'National Level College Entrance Examination for IITs',
    description: 'Cleared 2nd toughest exam in India with 28,593 rank out of 1,60,038 students with selection rate of ',
    icon : 'https://res.cloudinary.com/drg0umzcb/image/upload/v1729763472/shicoImages/sy9kxnhmlrankpwwvc1z.png'
  },
  {
    date: '2008-2022',
    title: 'St. Mary\'s School, India',
    location: 'High School',
    description: 'Completed my high school with 97.8% and senior secondary with 93.4% in CBSE board nationwide examination. ',
    icon : 'https://res.cloudinary.com/drg0umzcb/image/upload/v1729763536/shicoImages/c3r2svakhmdpzcfjnwad.png'
  },

];

export {experiencesData};